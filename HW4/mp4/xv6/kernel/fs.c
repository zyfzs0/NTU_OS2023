...
namex(char *path, int nameiparent, char *name)
{
 ...
 // printf("start new namei\n");
  ...
  while((path = skipelem(path, name)) != 0 && depth < 21){
    ...
    if(nameiparent && *path == '\0' && next == 0){
      // Stop one level early.
      // take care symbolic for name!
      iunlock(ip);
      return ip;
    }
    else if(nameiparent && *path == '\0' && next != 0){
    ...
    */
    ...
    /*
    // if next is file and depth>0
    if(ip->type == T_FILE && depth > 0 && depth < 10){
      // restore file ip
      ip = fip;
      path = org;
      memmove(path, copy, MAXPATH);
      break;
    }

    // if next is symbolic link
